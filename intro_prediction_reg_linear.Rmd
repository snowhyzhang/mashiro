...
有些模型有多个实现的包，这里使用了一些常用的包来建立相应的模型。  
本文的代码可以在[github](https://github.com/snowhyzhang/mashiro/blob/master/intro_prediction_reg_linear.Rmd)上找到。
...
```{r summary_rmse}
rmse_summary <- data.frame("Regression" = c("OLS", "rlm", "PCR", "PLS", "ridge", "lasso"), 
...
ggplot(rmse_summary, aes(x = Regression, y = RMSE)) + 
  ...
  # 由于差距不大，将y轴进行缩放，便于观察
  coord_cartesian(ylim = c(min(rmse_summary$RMSE - 0.05), 
